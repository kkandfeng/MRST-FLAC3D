%% This file only calculates Shi Qianfeng, which is convenient for different hole pressures
clc; clear;
...
p_ref   = 32 *mega*Pascal;                      % reference pressure
t_ref   = 136+273.15;                            % reference temperature
...
poro(             1 : n_onelayer*20) = 0.010;
poro(n_onelayer*21+1 : n_onelayer*29) = 0.1;
perm = ones(G.cartDims)*3.0;
...
perm(             1 : n_onelayer*20) = 0.03;
perm(n_onelayer*21+1 : n_onelayer*29) = 3;
ntg = ones(G.cartDims);
ntg(             1 : n_onelayer*20) = 1.0;
...
% inj_rate2 = 1.0e8 / year / co2_rho; % Áõ¼Ò¹µ×é 1.0e5 t /a        
% W = addWell(W, G, rock, wc2, 'type', 'rate', ...  
%             'val', inj_rate2, 'comp_i', [0 1]);  
        
wc3 = 70 * 70 * 28 + 70*30+31;      
inj_rate3 = 0.5e8 / year / co2_rho; 
W = addWell(W, G, rock, wc3, 'type', 'rate', ...  
            'val', inj_rate3, 'comp_i', [0 1], 'Radius', 1);            
...
initState.pressure = repmat(32e6 , Gt.cells.num, 1);
...
save('ShiQianFeng_mrst_sol.mat','Gt','states');
